...
class CreateRoom : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args[1].toDouble() !in minmoney..maxmoney){
            CoinFlip.sendMsg(sender,"§4金額は$minmoney~${maxmoney}にしてください")
            return true
        }

        if (CoinFlip.vault.getBalance(sender.uniqueId) < args[1].toDouble()){
        ...
        CoinFlip.coinFlipData[sender.uniqueId] = CoinFlipRoom(sender.uniqueId, args[1].toDouble() ,args[2] == "heads")//部屋データを入れる
